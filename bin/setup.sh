...
echo "  Prefer: ./bin/dev.sh"
echo "  Manual API (no TLS): cd api && source .venv/bin/activate && python -m uvicorn main:app --reload --host 0.0.0.0 --port 8000"
echo "  Spotify https redirect: ./bin/ssl-localhost.sh, then ./bin/dev.sh (do not mix plain uvicorn with https in .env.local)"
